...
- Every general collector constructs a `NormalizedMarketItem` and calls the same `market_flow.process_market_item(...)` facade. The facade selects compatibility or direct modules globally, then delegates every item to the shared decision/interpreter core; collectors no longer choose content/event flows or execute store/delivery branches.
