...
| `llm_rule_decision.py` | Report-only LLM decision contract: applies the confirmed company-disclosure/Sina-stock-news holding-only source boundary, selects rules only from an existing admitted `AdmissionResult`, accepts title/summary/body inputs with body code-bounded to its first 3,000 characters, identifies the applicable ordinary-strength rules, and strictly validates compact per-rule JSON, allowed actions and verbatim evidence before code mechanically aggregates the final action. The prompt and validator both limit each rule to three evidence or counterevidence quotes and each quote to 400 characters; a complete short body may be one such verbatim quote. It has no review, storage, delivery or dedup authority |
| `llm_rule_shadow.py` | Optional report-only LLM candidate: evaluates the same prepared production `NormalizedMarketItem`, calls the configured text model for an admitted item, validates the response through `llm_rule_decision.py`, and returns a bounded comparison candidate. If the first structurally valid response marks every degree rule not matched, it performs one bounded correction call under the same rule/evidence contract so the applicable ordinary-strength rule is evaluated. Model failure or invalid output produces no candidate action and remains visible as a model/validation failure |
