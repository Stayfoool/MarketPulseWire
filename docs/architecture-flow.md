...
| `rule_core_runtime_shadow.py` | Optional production report writer called after the current `DecisionResult` exists and before delivery; uses the exact production `NormalizedMarketItem`, caches explicit private configuration by file identity/mtime/size, removes source evidence text from output and cannot write reviews, reserve delivery keys or send messages |
