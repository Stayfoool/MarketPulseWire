...
| `rule_core_v1.py` | Inactive, side-effect-free candidate admission/decision core used only by the public v1 behavior corpus; bounded admission evidence includes all matched content-family evidence and global exclusion evidence, trade corridors preserve joint-or-two-sided matching, trusted attribution binds canonical ids to aliases/domains, and no production collector/runtime/store/delivery imports it |
...
The inactive v1 comparison remains report-only. Production collector batches may append bounded per-source comparison JSON and refresh `rule-core-shadow-combined-latest`; they do not change the current `DecisionResult` or delivery. When the private comparison switch is enabled, `surveil-rule-shadow-daily.timer` runs at 15:30 Beijing time and freezes `rule-core-shadow-daily-YYYY-MM-DD.md/json` for the preceding 24-hour 15:30 boundary. It sends one Feishu reminder only when that interval has comparable items or skipped new items, and a successful reminder prevents duplicate sends for the same report date. The authenticated Web workbench reads these dated JSON files through a bounded read-only API and exposes no rule, decision, configuration or delivery mutation from that view.
