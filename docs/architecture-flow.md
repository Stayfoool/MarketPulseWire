...
| `rule_core_shadow_daily.py` | Report-only daily review job; freezes one 15:30-to-15:30 Beijing Markdown/JSON report and sends at most one Feishu reminder when the interval has comparable or skipped items |
| `rule_shadow_report_store.py` | Bounded read-only loader for dated rule comparison reports used by the authenticated Web workbench; it cannot evaluate rules or send messages |
