...
The Web workbench exposes a lightweight authenticated `/api/health/summary` projection for its global Task Health badge. One batched read-only `systemctl show` call pairs each production timer with its execution service; the projection combines current logical-task failures with failing enabled source profiles and X connection-detail health. Shadow units, cut-over legacy units, the disabled-by-default JYGS path and disabled source profiles do not contribute to the count. The browser refreshes this summary only while visible, and the full Task Health view retains the detailed systemd/source rows and bounded log tails.
