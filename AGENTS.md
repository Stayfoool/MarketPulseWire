...
- LLM 在决策层内对已准入 `NormalizedMarketItem` 依据人工审定且当前启用的规则直接输出每条规则的 action 和最终 action，但不得新增规则、使用规则未定义的 action、修改规则含义或扩大来源/范围准入。所有产生 action 的 LLM 规则判断必须返回可逐字回验的原文证据和必要反证，并通过严格结构、规则适用范围、允许 action、版本和结果一致性校验；模型不可用、输出无效、证据回验失败或结果冲突时不得生成 `DecisionResult`，当前 review 必须标记为 `failed_retryable`，且不得回退到旧决策规则。结构和证据均有效但因决定 action 所需事实缺失、截断或相互冲突而返回的 `uncertain` 必须记录为 `insufficient_evidence`（证据不足终态），不生成 `DecisionResult`、不进入解释、投递或去重，也不得自动重试；`insufficient_evidence` 不是第四种 action。规则版本、prompt、模型、范围准入配置和代码版本必须可审计。决策后的 `market_interpreter` 仍只负责薄解读，不能另行创建或修改 action。
- 大模型决策规则的实际标题、action 条件、必需事实和排除条件不得进入 Git；只保存在本机项目目录内 Git 忽略且 mode `0600` 的私有规则文件和阿里云生产服务账号私有、mode `0600` 的对应文件中。Git 只保存规则格式、加载与严格校验代码以及不含生产规则语义的虚构测试配置。生产变更前必须核对本机与阿里云文件的规则版本和 SHA-256 摘要完全一致；部署不得上传、覆盖或删除该私有文件。
...
- 新增或调整由 LLM 输出 action 的规则判断时，必须使用固定假响应覆盖每个允许 action、未命中、不确定、模型不可用、无效 JSON、未知/遗漏/重复规则、未定义 action、原文证据回验失败、最终 action 不一致和提示词注入；普通 CI 不得调用真实 LLM。结构和证据均有效的 `uncertain` 必须关闭式进入 `insufficient_evidence`，其他生产失败必须关闭式进入 `failed_retryable`；两者都不得写入无效 `DecisionResult`、进入 delivery/dedup 或回退到旧决策规则。
