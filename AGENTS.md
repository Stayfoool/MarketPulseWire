...
- 大模型决策规则的实际标题、`push` 和 `daily` 条件不得进入 Git；只保存在本机项目目录内 Git 忽略且 mode `0600` 的私有规则文件和阿里云生产服务账号私有、mode `0600` 的对应文件中。Git 只保存规则格式、加载与严格校验代码以及不含生产规则语义的虚构测试配置。生产变更前必须核对本机与阿里云文件的规则版本和 SHA-256 摘要完全一致；部署不得上传、覆盖或删除该私有文件。
- LLM 程度决策的每次实际模型请求、原始回答、响应元数据和校验明细只可保存在阿里云服务账号私有、mode `0600` 的单条审计文件中。正常连续运行时，保留30天后必须删除敏感内容但保留有界决策结果；经批准的空数据库重建若明确放弃退出数据库批次的历史信息，必须将只属于该批次的大模型决策审计一并退出并在验证窗口后删除，只保留私有的文件数量、大小和 SHA-256 删除清单，不得保留审计正文副本。历史每日/合并对比报告和 Web API 不得包含完整模型输入、文章正文或原始回答；这些私有审计内容不得进入 Git、生产 SQLite、飞书或本机报告副本。
