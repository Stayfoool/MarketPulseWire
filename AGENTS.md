...
- 部署、systemd 服务、生产数据库、生产凭据和生产运行依赖以华为 Flexus 服务器为运行环境；生产配置仍以服务器 Web 面板和服务器私有 `.env` 为真源。
- 不为部署、生产运行或可选便利命令在 Mac 临时安装新的系统工具或 Python 包。`just` 等命令包装器不是项目正确性依赖；本机缺少时直接运行其底层命令，无法执行的依赖型测试交由必需 CI 和 Flexus 服务环境验证，并明确记录验证缺口。
