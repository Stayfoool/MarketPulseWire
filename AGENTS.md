...
- 优化必须优先保持系统简洁、架构一致和实际有效：先复用、收紧或删除现有规则和配置，不为单一案例新增并行流程、重复规则、补偿路径或额外运行负担。
- 持仓/关联关键词、重点主题、硬变量、宏观政策、归因研究和去重规则应优先设计为跨来源内容规则。同一内容只改变来源元数据时，通用决策原则上保持一致。
- `semiconductor_ai` 范围准入以半导体/AI产业链上游为主，另包括世界模型和大模型公司。支付、SaaS、光伏及其他一般下游应用或服务公司，不得仅因使用 AI、服务 AI 客户、被 AI 需求间接拉动或顺带提及大模型公司而进入该范围；范围关键词必须与当前发生变化的主体、产品、基础设施、模型或实质业务对象直接绑定。具体公司及后续增删由生产私有范围关键词和持仓/关联关键词管理，不得硬编码进代码或公共规则；提及这些公司不等于新闻主体就是这些公司。持仓及关联关键词形成的 `holding` 范围不受本条限制。
- `macro_data` 范围准入和程度决策只保留人工审定的核心宏观指标；核心指标名单以生产私有配置中的 `macro_primary_indicators` 为真源。次级指标不得进入 `macro_data` 范围，也不得保留或新增次级指标程度规则、市场反应例外、特殊阈值或其他补偿路径。
