...
def test_analyze_event_returns_interpretation_without_writing_legacy_tables() -> None:
    ...
    assert analysis["decision_result"]["action"] == "push"
    assert analysis["interpretation_result"]["core_content"].startswith("美国 CPI")
    assert analysis["interpretation_result"]["model"] == "fake-model"
    assert analysis["interpretation_result"]["brief_reason"] == ""
    assert analysis["interpretation_result"]["related_targets"] == []
