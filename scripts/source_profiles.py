...
def build_profiles() -> list[SourceProfile]:
    profiles: list[SourceProfile] = [
        SourceProfile(
            ...
            category="x_serenity",
            name="X / Serenity",
            source_type="X 浏览器时间线",
            fetch_range="已登录 X 页面“正在关注”时间线中可见的推文；只读取有限 DOM 状态",
            filter_policy="技术去重后进入统一五组范围准入和大模型程度决策",
            frequency="每 10 分钟 timer",
            runtime_shape="timer one-shot",
            pipeline="X 浏览器采集 -> NormalizedMarketItem -> 统一决策/解读 -> market_reviews -> 统一投递/view",
            service_units=("surveil-x-browser-collector.timer", "surveil-x-browser-collector.service"),
            health_keys=(("x_browser", "x_serenity"),),
            fetcher="scripts/x_browser_monitor.py -> scripts/value_directory_browser.py",
            proxy_profile="通常走服务器本机 mihomo 代理",
            notes="使用服务器私有 Chromium profile；首次登录由操作员完成，collector 不导入/导出 Cookie。",
        )
    ]
    ...
    for page_source in TREND_FORCE_PAGE_SOURCES:
        profiles.append(
            SourceProfile(
                ...
                runtime_shape="timer one-shot",
                pipeline="NormalizedMarketItem -> 统一决策/解读 -> market_reviews -> 统一投递/view",
                service_units=("surveil-research-collector.timer", "surveil-research-collector.service"),
    ...
    profiles.append(
        SourceProfile(
            ...
            frequency="每天 05:00 / 21:00 timer；首次登录后手动启用",
            runtime_shape="timer one-shot / server browser profile",
            pipeline="价值目录浏览器/首页预览/OCR -> NormalizedMarketItem -> 统一决策/解读 -> market_reviews -> 统一去重/投递/view",
    ...
    profiles.append(
        SourceProfile(
            ...
            frequency="每天 05:00 / 21:00 timer；与价值目录个股同一服务运行",
            runtime_shape="timer one-shot / server browser profile",
            pipeline="价值目录浏览器/首页预览/OCR -> NormalizedMarketItem -> 统一决策/解读 -> market_reviews -> 统一去重/投递/view",
            service_units=("surveil-value-directory.timer", "surveil-value-directory.service"),
    ...
    for source_id in CORE_COMPANY_FEEDS:
        ...
        profiles.append(
            SourceProfile(
                ...
                frequency="每 10 分钟 timer",
                runtime_shape="timer one-shot",
                pipeline="官网来源 -> NormalizedMarketItem -> 统一决策/解读 -> market_reviews -> 统一投递/view",
                ...
                url=url,
                notes="公司官网源属于一手信息，统一结果保存到 market_reviews。",
            )
        )
    ...
    for policy_source in TRADE_POLICY_SOURCES:
        profiles.append(
            SourceProfile(
                ...
                runtime_shape="timer one-shot",
                pipeline="官方贸易政策采集 -> NormalizedMarketItem -> 统一决策/解读 -> market_reviews -> 统一去重/投递/view",
    ...
    for source_id, url in CHINA_MEDIA_FEEDS.items():
        ...
        profiles.append(
            SourceProfile(
                ...
                runtime_shape="timer one-shot",
                pipeline="新闻媒体来源 -> NormalizedMarketItem -> 统一决策/解读 -> market_reviews -> 统一投递/view",
                service_units=("surveil-news-collector.timer", "surveil-news-collector.service"),
                health_keys=(
    ...
    profiles.extend(
        [
            SourceProfile(
                ...
                pipeline="快讯采集 -> NormalizedMarketItem -> 统一决策/解读 -> market_reviews -> delivery/view",
