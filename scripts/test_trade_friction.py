...
from trade_friction import RULE_ID, classify_trade_friction
...
def test_section_301_public_comment_matches_across_sources() -> None:
    text = "USTR seeks public comment on a proposed Section 301 tariff action concerning China semiconductor imports."
    rules = [classify_trade_friction(item) for item in variants(text)]
...
def test_eu_china_ev_anti_subsidy_investigation_pushes() -> None:
    ...
    rule = classify_trade_friction({"title": text})
...
def test_cordial_bilateral_meeting_does_not_match() -> None:
    ...
    assert classify_trade_friction({"title": text}) is None
...
def main() -> int:
    test_section_301_public_comment_matches_across_sources()
