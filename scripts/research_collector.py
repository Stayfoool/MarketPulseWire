#!/usr/bin/env python3
"""Shadow collector for research institutions and industry media.

This is the first consolidation step for the research/industry-media source
family. It intentionally does not send Feishu cards, does not run LLM review,
and does not write production dedupe/review tables. It only fetches and reports
what the future unified collector would see.
"""
...
from rss_monitor import CORE_COMPANY_FEEDS, DB_PATH, fetch_feed, filter_items, strip_tags
...
def write_report(payload: dict[str, Any], report_dir: Path = REPORT_DIR) -> Path:
    ...
    path = report_dir / f"research-collector-shadow-{stamp}.json"
...
def main() -> int:
    load_env(ENV_PATH)
    parser = argparse.ArgumentParser(description="Shadow-run research/industry-media collector.")
