...
LLM_DECISION_RULE_VERSION = "llm-decision-rules-v15-20260725"
...
RULES: tuple[LLMRuleDefinition, ...] = (
    ...
    _rule(
        ...
        push=(
            ...
            "正式订单、采购、约束性供货或首次确认向全球大厂供货。已配置关联新闻关键词命中同行、产品或产业链对象时，"
            "若原文给出多项相互印证的当前重大行业硬事实，例如供需明确偏紧或主力企业满产，并有重大扩产、减产、长期锁量"
            "订单或相对现有产能的显著倍数锁定，且能明确说明对持仓所在产品或产业链供需、价格、收入、成本或竞争格局的"
            "实质影响，也可push；单个订单主体未具名不妨碍由明确行业对象、当前状态、重大量级和持仓影响路径共同成立。"
            "关联新闻关键词只证明关联，单独命中不能push。以下股份事项均为push，不设金额或比例阈值，"
        ...
        required=(
            "持仓主体",
            "直接企业变化对象，或已配置关联公司、产品、产业链对象及其与持仓的影响路径；股份交易还须局部绑定行为主体和该持仓公司自身股份，回购行为主体可以是持仓公司本身",
